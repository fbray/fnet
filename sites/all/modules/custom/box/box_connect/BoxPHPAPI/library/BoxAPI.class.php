<?php 
	...
	class Box_API {
		...
		// Deprecated by changes in the Box API. Here for reference.
		// public $authorize_url 	= 'https://www.box.com/api/oauth2/authorize';
		//public $authorize_url 	= 'https://api.box.com/oauth2/authorize';


		// public $token_url	 	= 'https://www.box.com/api/oauth2/token';
		public $token_url 		= 'https://app.box.com/api/oauth2/token';
		public $api_url 		= 'https://api.box.com/2.0';
		public $upload_url 		= 'https://upload.box.com/api/2.0';

		public function __construct($client_id = '', $client_secret = '', $redirect_uri = '') {
			if(empty($client_id) || empty($client_secret)) {
				drupal_set_message('Invalid CLIENT_ID or CLIENT_SECRET or REDIRECT_URL. Please provide CLIENT_ID, CLIENT_SECRET and REDIRECT_URL when creating an instance of the class.');
			} else {
		...
		public function get_code() {
			if(array_key_exists('refresh_token', $_REQUEST)) {
				...
				$url = $this->authorize_url . '?' . http_build_query(array('response_type' => 'code', 'state' => 'my_state', 'client_id' => $this->client_id, 'redirect_uri' => $this->redirect_uri));
				header('location: ' . $url);
				exit();
			}
		}
